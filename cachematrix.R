...
makeCacheMatrix <- function( x = matrix() ) 
{
  ...
  get <- function()   # return matrix passed to (x) - trivial
  {
    x
  }

  setInverse <- function(inverse)   # ontain inverso y retorna
  {
  	retorno <<- inverse
  	retorno
  }

  getInverse <- function()  # return the inverse var (m)
  {
  	m
  }
...
cacheSolve <- function(x, ...) 
{
  ...
  {
    temporal <- x$get()
    m        <- solve(temporal) %*% temporal   # the real vudu magic
